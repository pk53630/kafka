...
for file in files: 
    ...
    for i in range(0,len(val_lst)):
        ...
        #print(TIME_VALUE)
        TIME_VALUE_AM_PM = val_lst[i][1].split(" ")
        #print(TIME_VALUE_AM_PM[1])
        #print(DIVISION_ID)
        DATE_TIME_i = "to_date('"+str(DATE_VALUE)+" "+str(TIME_VALUE)+"','MM/DD/YYYY HH:MI:SS "+TIME_VALUE_AM_PM[1]+"')"
        DATE_TIME=DATE_TIME_i.replace('"', '')
        #print(DATE_TIME)
        cursor.execute("SELECT "+DATE_TIME+" FROM DUAL")
        row_date = cursor.fetchone()
        #print(row_date[0])
        val_lst[i].insert(59,DIVISION_ID)
        val_lst[i].insert(60,row_date[0])
        val_lst[i].insert(61,row_date[0])
        ...
        cursor.execute("""
        insert into iprod_dba.echometer_fluid_survey (ACOUSTIC_VEL,ACOUSTIC_VEL_CALC_METHOD,CASING_PRESSURE,JOINTS_PER_SECOND,
